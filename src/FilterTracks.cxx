...
void FilterTracks::processEvent(LCEvent* evt) {
  ...
  for (int i = 0; i < InTrackCollection->getNumberOfElements(); i++) {
    ...
    int nhitvertex =
        trk->getSubdetectorHitNumbers()[1] + trk->getSubdetectorHitNumbers()[2];
    int nhitinner =
        trk->getSubdetectorHitNumbers()[3] + trk->getSubdetectorHitNumbers()[4];
    int nhitouter =
        trk->getSubdetectorHitNumbers()[5] + trk->getSubdetectorHitNumbers()[6];

    float pt = fabs(0.3 * _Bz / trk->getOmega() / 1000);

    if (nhittotal > _NHitsTotal and nhitvertex > _NHitsVertex and
        nhitinner > _NHitsInner and nhitouter > _NHitsOuter and pt > _MinPt) {
